...
exports.findOne = (req, res) => {
    centerBoard.findById(req.params.centerBoardId)
    .then(centerBoard => {
        if(!centerBoard) {
...
exports.update = (req, res) => {
    ...
    centerBoard.findByIdAndUpdate(id,updateData,{new: true})
    .then(centerBoard => {
        if(!centerBoard) {
            return res.status(404).send({
                message: "centerBoard not found with id " + req.params.centerBoardId
...
exports.delete = (req, res) => {
    centerBoard.findByIdAndRemove(req.params.centerBoardId)
    .then(centerBoard => {
        if(!centerBoard) {
            return res.status(404).send({
                message: "centerBoard not found with id " + req.params.centerBoardId
